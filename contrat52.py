...
def open_virement_selection_window():
    ...
    # 📌 Filtrer pour mars 2025
    df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
    df_filtered = df[df["Date"].dt.strftime("%Y-%m") == "2025-03"]

    if df_filtered.empty:
        messagebox.showwarning("Aucun virement", "Aucun virement trouvé pour mars 2025.")
        return
 

    # 📌 Créer la fenêtre de sélection des virements
    virement_window = tk.Toplevel()
    virement_window.title("Sélection des virements - Mars 2025")
    ...
    for _, row in df_filtered.iterrows():
        tree.insert("", "end", values=(row["Beneficiaire"], row["IBAN"], row["Date"].strftime("%Y-%m-%d"), row["Reference"], f"{row['Montant']:.2f}"))
    ...
    def generate_selected_virements():
        ...
        fichier_xml = generer_xml_virements(selected_virements)
        if fichier_xml:
